...
expedia_dest = expedia_dest[!(is.na(expedia_dest$srch_destination_latitude)),]
expedia_dest.cut = expedia_dest[,1:5]
...
expedia_data.usa$srch_destination_id <- as.integer(expedia_data.usa$srch_destination_id)
expedia_dest.usa$srch_destination_id <- as.integer(expedia_dest.usa$srch_destination_id)
expedia_data.usa.merged <- merge(expedia_data.usa,expedia_dest.usa,by="srch_destination_id", all.x = T)

temp = data.frame(aggregate(is_booking ~  user_location_region + region + month, expedia_data.usa.merged, length))
colnames(temp) = c("user_location_region", "srch_destination", "month", "srch_count")
temp$is_booking = 0
data.aggre = bind_rows(data.aggre, temp)
temp = data.frame(aggregate(is_booking ~ user_location_region + region + month, expedia_data.usa.merged, sum))
colnames(temp) = c("user_location_region", "srch_destination", "month", "srch_count")
temp$is_booking = 1
data.aggre = bind_rows(data.aggre, temp)
...
expedia_data.canada$srch_destination_id <- as.character(expedia_data.canada$srch_destination_id)
expedia_dest.canada$srch_destination_id <- as.character(expedia_dest.canada$srch_destination_id)
expedia_data.canada.merged <- merge(expedia_data.canada,expedia_dest.canada,by="srch_destination_id", all.x = TRUE)

temp = data.frame(aggregate(is_booking ~  user_location_region + region + month, expedia_data.canada.merged, length))
colnames(temp) = c("user_location_region", "srch_destination", "month", "srch_count")
temp$is_booking = 0
data.aggre = bind_rows(data.aggre, temp)
temp = data.frame(aggregate(is_booking ~ user_location_region + region + month, expedia_data.canada.merged, sum))
colnames(temp) = c("user_location_region", "srch_destination", "month", "srch_count")
temp$is_booking = 1
data.aggre = bind_rows(data.aggre, temp)
...
expedia_dest.usa$srch_destination_latitude <- as.numeric(as.character(expedia_dest.usa$srch_destination_latitude))
expedia_dest.usa$srch_destination_longitude <- as.numeric(as.character(expedia_dest.usa$srch_destination_longitude))
...
expedia_dest.canada$srch_destination_latitude <- as.numeric(as.character(expedia_dest.canada$srch_destination_latitude))
expedia_dest.canada$srch_destination_longitude <- as.numeric(as.character(expedia_dest.canada$srch_destination_longitude))
...
mergedata <- merge(mergedata, locations, by.x = "srch_destination", by.y = "region", all.x = T)
colnames(mergedata)[8:9] = c("latitude","longitude")

write.csv(mergedata, file="data_aggregate.csv", row.names = FALSE)
